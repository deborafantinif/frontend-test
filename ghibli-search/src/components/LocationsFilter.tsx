...
import { ILocation } from '../interfaces/endpoints';
...
function LocationsFilter({setIsMoreFiltersSelected, allLocations, fetchFilms, requestWithFilter}: ILocationsFilterProps) {
  ...
  function filteringLocations() {
    const filteredPeople = allLocations
      ...
      .filter((location) =>
        location.climate.toLowerCase().includes(climateName.toLowerCase())
      )
      .filter((location) =>
        location.terrain.toLowerCase().includes(terrainName.toLowerCase())
      )
      .filter(
        (location) =>
          Number(location.surface_water) >= Number(minWater) &&
          Number(location.surface_water) <= Number(maxWater)
      );
      ...
      <form className={styles.filtersForm}>
        ...
        <div className={styles.filtersInputNumber}>
          <input
            type="number"
            name="max-water"
            placeholder="Minimum water"
            onChange={(e) => setMinWater(e.target.value)}
          />
          <input
            type="number"
            name="min-water"
            placeholder="High water"
